...
action :mklabel do
  execute "parted #{new_resource.device} --script -- mklabel #{new_resource.label_type}" do
    ...
    not_if "parted #{new_resource.device} --script -- print |grep -E 'Partition Table: (gpt|msdos|#{new_resource.label_type})'"
  end
end
...
#{new_resource.part_start} #{new_resource.part_end}" do
    ...
    not_if "parted #{new_resource.device} --script -- print |grep -E '(#{new_resource.part_type}|#{new_resource.file_system})'"
  end
end
