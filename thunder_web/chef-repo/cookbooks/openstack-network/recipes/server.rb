...
# copy oslo package to local directory
py_package_dir = node['openstack']['py_package_dir']
py_lc_package_dir = py_package_dir.gsub('/lib/', '/local/lib/')
execute 'fix neutron python packages dependency' do
  cwd "#{py_package_dir}/oslo/"
  command "cp -r #{py_package_dir}/oslo/rootwrap/ #{py_lc_package_dir}/oslo/"
  action :run
  only_if { !File.directory?("#{py_lc_package_dir}/oslo/rootwrap/") }
end
...
# commented to fix the error network table existing issue
