...
include_recipe 'thunder-setup::block_storage_setup'
  ...
  if node['openstack']['block-storage']['volume']['create_volume_group']
    ...
    when 'block_devices'
      ...
      node['thunder']['node_disk_list'].each do |disk_info|
        ...
        # if disk is not used for os installation and if not system space
        if (disk_info['device'] != node['thunder']['os_installation_disk']) && (disk_info['system'] != 1)
      
          # commented to override dynamically from node conf
          #block_devices = node['openstack']['block-storage']['volume']['block_devices']
          
          block_devices = block_devices + " " + disk_info['part_name']
