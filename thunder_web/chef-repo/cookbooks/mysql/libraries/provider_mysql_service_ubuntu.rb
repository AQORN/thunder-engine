    ...
    class MysqlService
      class Ubuntu < Chef::Provider::MysqlService
        ...
        action :create do
          ...
          # get old pass
          old_db_pass = ''
          db_root_pass = Shellwords.escape(new_resource.parsed_server_root_password)
          
          # check root marker file exist
          if ::File.exist?('/etc/.mysql_root')
            old_db_pass = ::File.open('/etc/.mysql_root').read.chomp
          end
          
          # check whether old pass string is empty
          if old_db_pass.to_s == ''
            old_db_pass_str = ""
          else
            old_db_pass_str = "-p" + old_db_pass
          end
          
          # if new password different from old password
          if old_db_pass != db_root_pass
            execute 'assign-root-password' do
              ...
              command cmd
              action :run
              only_if "#{prefix_dir}/bin/mysql -u root #{old_db_pass_str} -e 'show databases;'"
            end
          end           
          ...
          # create pass string for grant options
          if db_root_pass.to_s == ''
            db_pass_str = ''
          else
            db_pass_str = "-p" + db_root_pass
          end
          ...
          execute 'install-grants' do
            ...
            cmd << ' -u root '
            cmd << "#{db_pass_str} < /etc/mysql_grants.sql"
            command cmd
            action :nothing
