...
admin_email = node['openstack']['identity']['admin_email']

# get required values and assign it
...
node['openstack']['identity']['users'].values.map do |user_info|
  ...
  openstack_identity_register "Grant '#{role_name}' Role to '#{username}' User in '#{tenant_name}' Tenant" do
    auth_uri auth_uri
    bootstrap_token bootstrap_token
    user_name username
    role_name role_name
