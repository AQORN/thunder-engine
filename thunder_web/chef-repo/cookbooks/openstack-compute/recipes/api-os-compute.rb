...
execute 'fix nova python packages dependency' do
  cwd "#{py_package_dir}/nova/api"
  command "wget #{node['thunder']['pkg_download_url']}/compute_req_id.py"
  action :run
  only_if { !File.exists?("#{py_package_dir}/nova/api/compute_req_id.py") }
end
